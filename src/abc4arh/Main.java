...
/** 
 *  Runner code for ARTIFICIAL BEE COLONY ALGORITHM FOR ASSOCIATION RULE HIDING
 *  (ABC4ARH)
...
class Main {
    public static void main(String[] args) throws IOException {
        System.out.println("Running ABC4ARH algorithm.");
        /* 
         * Parameters:
         * input file path
         * SAR file path
        ...
        for (int inputRun = 0; inputRun < inputNamesSAR.length; inputRun++) {
            ...
            String inputWeak = directory + dataset + "/rules/" + dataset + "_wr.txt";

            for (int i = 0; i < NUM_RUNS; i++) {
                System.out.println("############ ABC4ARH RUN no." + i + " ############");
                System.out.println("DATASET: " + dataset + "; INPUT TYPE: " + outputNames[inputRun]);
                String output = directory + dataset + "/measurements/abc4arh/abc4arh_" + outputNames[inputRun] + "_sanitized_0" + i + ".txt";
                new ABC4ARH().runAlgorithm(input, inputSAR, inputNAR, inputWeak, output, minsup, minconf);
            }
        }
    }
}
